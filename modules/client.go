...
func (c *Client) Vendor() error {
	...
	for _, t := range tc.AllModules {
		...
		for _, mount := range t.Mounts() {
			if err := hugio.CopyDir(c.fs, filepath.Join(dir, mount.Source), filepath.Join(vendorDir, t.Path(), mount.Source), shouldCopy); err != nil {
...
func (c *Client) graph() (string, error) {
	if c.GoModulesFilename == "" {
		return "", nil
	}

	var b bytes.Buffer
	err := c.runGo(context.Background(), &b, "mod", "graph")
	if err != nil {
...
func (c *Client) rewriteGoModRewrite(name string, isGoMod map[string]bool) ([]byte, error) {
	...
	// TODO(bep) mod this works, but add some level of tests.
	if name == goModFilename {
		isModLine = func(s string) bool {
			return !strings.Contains(s, "require (") && (strings.HasPrefix(s, "require ") || strings.HasPrefix(s, "\t"))
		}
	}
	...
	for scanner.Scan() {
		...
		if isModLine(line) {
			modname := strings.TrimPrefix(line, "require ")
			modname = strings.TrimSpace(modname)
			modname = strings.TrimSuffix(modname, "// indirect")
			if modname == "" {
				doWrite = true
			} else {
